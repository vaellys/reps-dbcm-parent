...
statusCode:{ok:200,fail:205,error:500,timeout:504,warn:100,nologin:203},
domain:"",
ui:{sbar:true},
frag:{},
_msg:{},
...
if(messager){
messager.error($.regional.messager.ajaxError);}else{
alert($.regional.messager.ajaxError);}
if(window.console){
console.log("<div>Http status: "+xhr.status+" "+xhr.statusText+"</div>"+"<div>ajaxOptions: "+ajaxOptions+"</div>"+"<div>thrownError: "+thrownError+"</div>"+"<div>"+xhr.responseText+"</div>");}},
ajaxDone:function(json){
if(json.statusCode==REPS.statusCode.error){
if(json.message&&messager)messager.error(json.message);}else if(json.statusCode==REPS.statusCode.timeout){
if(messager){
messager.error(json.message);}}else{
...
fileSize=0;
jThis.containFileType=true;}else{
...
var val=json[key];
$appendTr.append("<td style='"+$(this).attr("style")+"'><div><span>"+val+"</span></div></td>");});}else{
$thead.find("tr:first th").each(function(i){
var key=$(this).find("div").html();
var val=json[key];
...
messager.info(json.message);}}else if(json.statusCode==REPS.statusCode.fail){
if(json.message&&messager){
messager.info(json.message);}}else if(json.statusCode==REPS.statusCode.warn){
if(json.message&&messager){
messager.warn(json.message);}}else{
if(json.message&&messager)
...
if(confirmMsg){
messager.confirm(confirmMsg,{okCall:_submitFn});}else{
_submitFn();}
return false;}
...
isRepsFormSubmitRepeat=false;}else if(json.statusCode==REPS.statusCode.timeout){
if(json.message&&messager){
messager.info(json.message);}
isRepsFormSubmitRepeat=false;}else if(json.statusCode==REPS.statusCode.fail){
if(json.message&&messager){
messager.info(json.message);}
isRepsFormSubmitRepeat=false;}else if(json.statusCode==REPS.statusCode.warn){
if(json.message&&messager){
messager.warn(json.message);}
isRepsFormSubmitRepeat=false;}else{
if(json.message&&messager){
messager.correct(json.message,{
okCall:okcall||function(){
if(redirect){location.href=redirect;}}});}};},
...
if(json.statusCode==REPS.statusCode.error){
if(json.message)messager.error(json.message);}else{
...
gh=iRefH;}
var iH=gh-50;
var $panelHeader=$(this).parent().parent().siblings(".panel-header");
if($panelHeader.size()>0){
iH-=27;}
if($this.isTag("table")){
$this.removeAttr("layoutH").wrap('<div style="overflow:auto;height:'+iH+'px"></div>');}else{
if(!flagNoHeight){
$this.height(iH).css("overflow","auto");}}});},
...
isRepsFormSubmitRepeat=true;}else{
isRepsFormSubmitRepeat=false;}});});(function($){
...
leftWidth=$panel.width();}else if(setting.dock=="right"){
rightWidth=$panel.width();}else if(setting.dock=="bottom"){
bottomHeight=$panel.height();}else if(setting.dock=="top"){
...
if(width){
$("div.gridScroller",this).width(width+"px");}});});})(jQuery);(function($){
if($.validator){
...
return this.optional(element)||/^(\d{3,4}-)?\d{7,8}$/.test(value);},"Please specify a valid phone number");
$.validator.addMethod("contactinformation",function(value,element){
return this.optional(element)||valiedateContactInformation(value);},"Please specify a valid contact information");
$.validator.addMethod("mobile",function(value,element){
return this.optional(element)||/^((\(\d{3}\))|(\d{3}\-))?13\d{9}|14[57]\d{8}|15\d{9}|18\d{9}$/.test(value);},"Please specify a valid mobile");
$.validator.addMethod("chinese",function(value,element){
return this.optional(element)||/[\u4e00-\u9fa5]/.test(value);},"Please specify a valid chinese");
$.validator.addMethod("password",function(value,element){
return this.optional(element)||/^[\S]{6,18}$/.test(value);},"Please specify a valid password");
$.validator.addMethod("identifycode",function(value,element){
return this.optional(element)||/(^\d{15}$)|(^\d{18}$)|(^\d{17}(\d|X|x)$)/.test(value);},"Please specify a valid identifycode");
$.validator.addMethod("postcode",function(value,element){
return this.optional(element)||/^[0-9]{6}$/.test(value);},"Please specify a valid postcode");
...
return this.optional(element)||/^(0|[1-9]\d*)$/.test(value);},"Integer numbers only please");
$.validator.addMethod("chinesealphanumeric",function(value,element){
return this.optional(element)||/^[\u4e00-\u9fa5a-zA-Z0-9_]+$/.test(value);},"chinese or alphanumeric only please");
$.validator.addMethod("filename",function(value,element){
var fileNameTest=/^[^\\\/:*?"<>|]+/;
return this.optional(element)||fileNameTest.test(value);},"Please enter a valid filename.");
$.validator.addMethod("moneyvalidate",function(value,element){
return this.optional(element)||/^\d+(\.\d+)?$/.test(value);},"Please enter a valid number.");
$.validator.addMethod("idcard",function(value,element){
return isIDCard(value);},"Please enter a valid idcard.");
...
sfzh=sfzh.substring(0,6)+"19"+sfzh.substring(6);}else if(parseInt(sfzh.length)==18){
sfzh=sfzh.left(17);}
if(!isInt(sfzh)){
return false;}
...
sjjg=sjjg+nA[i]*nW[i];
i=i+1;}
return zjbh15+sJYMDZB[sjjg%11];}})(jQuery);
...
$.extend(op,options);
var buttons=[{name:op.okName,call:op.okCall,keyCode:REPS.keyCode.ENTER}];
...
$.extend(op,options);
var buttons=[{name:op.okName,call:op.okCall,keyCode:REPS.keyCode.ENTER},{name:op.cancelName,call:op.cancelCall,keyCode:REPS.keyCode.ESC}];
this._open(this._types.confirm,msg,buttons);},
message:function(data,callback){
if(data.statusCode==REPS.statusCode.ok){
this.correct(data.message,{okCall:callback});}else if(data.statusCode==REPS.statusCode.error){
...
accordion[options].apply(accordion,args);}else if(!$(this).is(".reps-accordion"))
...
h=$(obj).find(".accordionContent").height()}else{
h=parent.height();}
...
if(flagSeq){
if(showSeq){
oldThs=$table.find("thead>tr:last-child").prepend("<th align='"+(flagAlign==null?"center":flagAlign)+"' flagSeq=\"true\" width=\""+seqWidth+"\">序号</th>").find("th");}else{
oldThs=$table.find("thead>tr:last-child").prepend("<th flagSeq=\"true\" width=\""+seqWidth+"\"></th>").find("th");}}else{
...
var oriWidth;
if(0==i){
oriWidth=(seqWidth/sum)*tlength;}else{
oriWidth=(widths[i-1]/sum)*tlength;}
width=oriWidth-(100*oriWidth/tlength)-2;}else{
if(widths.length<=0){
width=$th.width()-(100*$th.width()/tlength)-2;}else{
...
$("li.j-num").eq(i).find("a").trigger("click");});}else{
...
alert($.regional.pagination.currentPage);}else{
...
$(".selected.j-num").find("a").trigger("click");}}else{
...
$(".pagination-nav").attr("totalCount",newPaginationTotalCount);}else{
...
if(opts.end){
$form.find("input[name='curPageNumber']").val(opts.curPageNumber);}else{
...
return str.substr(str.length-len);}})(jQuery);(function(){
function addEvent(el,type,fn){
if(el.addEventListener){
el.addEventListener(type,fn,false);}else if(el.attachEvent){
...
if(button.jquery){
button=button[0];}else if(typeof button=="string"){
if(/^#.*/.test(button)){
...
var settings={id:"",path:"",url:"",action:"../../common/upload.mvc",callback:"",flagAbsolute:false,fileType:null,size:0,reName:null,fileName:"",coverage:false,prefixName:"",suffixName:""};
...
var imagePath=REPS.domain+"/theme/default/images/loading.gif";
...
var call="var responseCallback = "+opts.callback;
eval(call);
responseCallback(fileName,rFileType,fileSize,filePath,options.id);}}else if(response.statusCode==REPS.statusCode.warn){
messager.message(response,function(){});
return;}else{
return;}
REPS.containFileType=false;
REPS.overFileSizeFlag=0;}});}})();(function($){
$.pdialog={
...
if(buttonFlag){
jBContent.html(appendIframeAndButtons);}else{
...
return false;});
if(""!=op.afterLoad){
$.pdialog.afterLoad(op.afterLoad);}}else if(url&&url!='undefined'){
if(""!=op.beforeLoad){
$.pdialog.beforeLoad(op.beforeLoad);}
$.ajax({
type:'GET',
...
return false;});
if(""!=op.afterLoad){
$.pdialog.afterLoad(op.afterLoad);}}});}else{
var content;
if(""!=op.blockId){
content=$("#"+op.blockId).html();}else{
...
if(buttonFlag){
$(".dialogContent",dialog).height(height-$(".dialogHeader",dialog).outerHeight()-$(".dialogButtons",dialog).outerHeight());}else{
...
window.parent.$(".dialogHeader .close").click();}};})(jQuery);(function($){
...
initTab(jT);
return false;});}else if($(this).hasClass(op.ajaxIframeClass)){
var jGroup=jGroups.eq(iTabIndex);
...
jGroup.attr("inited",1000).attr("style",jT.attr("style")).find("input[type=text]").filter("[alt]").inputAlert();}}}});})(jQuery);
